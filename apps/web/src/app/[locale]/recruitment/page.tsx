...
const TEAMS: Team[] = [
  ...
  },
  {
    ...
    preferred: ["會使用 Illustrator / Figma / Photoshop 等設計軟體", "有作品集尤佳"],
    process: [
      "即日起 ~ 8/16（六） 報名截止",
      "8/17（日）~ 8/19（二） 書審",
      "8/20（三）前寄送通過 / 未通過信件",
  ...
  },
  {
    ...
    techStack: ["HTML", "CSS", "JavaScript", "Astro", "Fastify", "Three.js", "Figma"],
  },
  {
    id: "record",
    name: "紀錄組",
    ...
    duties: ["年會當天拍攝議程、互動、攤位等", "活動前參與培訓，後期協助修圖與整理"],
    preferred: [
      "有攝影經驗與基礎器材",
      "具備構圖與調色概念",
...
export default function RecruitPage() {
  return (
    ...
    <main id="top" className="min-h-screen w-full bg-gradient-to-b from-[#1A2316] to-[#121712] text-white my-12">

      <div className="mx-auto max-w-6xl px-4 py-8 mb-8">
        {/* Header */}
        <header className="mb-8 py-15 text-center">
          <h1 className="text-3xl sm:text-4xl md:text-4.5xl font-bold">SITCON 2026 <br />工作人員招募資訊</h1>
        ...
        <section className="md:hidden space-y-4">
          {TEAMS.map((t) => (
            <article key={t.id} className="rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur">
          ...
          <div className="overflow-hidden rounded-xl border border-white/10">
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-white/10">
                {TEAMS.map((t, idx) => (
                  <tr key={t.id} className={idx % 2 === 0 ? "bg-white/0" : "bg-white/[0.03]"}>
                    <Td>
                    ...
                    <Td>
                      <div className="space-y-3">
                        {t.preferred && t.preferred.length > 0 && (
                          <section>
                            <h4 className="mb-1 text-xs font-semibold text-emerald-300/90">推薦條件</h4>
                            <BulletList items={t.preferred} />
                          </section>
                        )}
                        {t.process && t.process.length > 0 && (
                          <section>
                            <h4 className="mb-1 text-xs font-semibold text-emerald-300/90">招募流程</h4>
                            <BulletList items={t.process} />
                          </section>
                        )}
                        {t.techStack && t.techStack.length > 0 && (
                          <section>
                            <h4 className="mb-1 text-xs font-semibold text-emerald-300/90">技術 / 工具</h4>
                            <TagList items={t.techStack} />
                          </section>
                        )}
                      </div>
...
function Td({ children, className = "" }: React.PropsWithChildren<{ className?: string }>) {
  return <td className={`align-top px-4 py-4 text-sm text-gray-100 ${className}`}>{children}</td>;
}
